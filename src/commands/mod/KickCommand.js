...
module.exports = class BanCommand extends BaseCommand {
  ...
  async run(client, message, args) {
    ...
    if (!message.member.hasPermission("KICK_MEMBERS")) return message.channel.send("This command is restricted to Jantick Staff.");
    ...
    if (person.id === message.author.id) return message.channel.send("You can not kick yourself.");
  
    if (message.member.roles.highest.position <= message.guild.members.cache.get(person.id).roles.highest.position || person.id === message.guild.owner.id) return message.channel.send("That user has a higher role ranking, meaning you can not kick them");
